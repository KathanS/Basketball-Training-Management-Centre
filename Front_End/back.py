...
conn = psycopg2.connect(
   database="Basketball", user='postgres', password='Bhavin312', host='127.0.0.1', port= '5432'
)
...
def in_info(mail, address, m1, m2 ,des):
    conn = psycopg2.connect(database="Basketball", user='postgres', password='Bhavin312', host='127.0.0.1', port= '5432')
    cursor = conn.cursor()
    conn.autocommit = True
...
def query5(): 
    ...
    cursor = conn.cursor()
    conn.autocommit = True
    cursor.execute(''' select avg("Age") from "Sports_Training"."Player"''')
    result = cursor.fetchall();
